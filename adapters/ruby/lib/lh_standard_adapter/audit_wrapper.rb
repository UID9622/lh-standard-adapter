...
module LhStandardAdapter
  ...
  class AuditWrapper
    ...
    def wrap(payload, task_type: "default", persona: "P04")
      ...
      color = determine_color(pattern, sig["R"])
      payload_json = JSON.generate(payload)
      payload_hash = Digest::SHA256.hexdigest(payload_json)[0,16]

      {
        "audit_version"=>"v1.0","uid"=>"UID#{@uid}","persona"=>persona,"task_type"=>task_type,
        "behavior_signature"=>sig,"behavior_pattern"=>pattern,"behavior_labels"=>labels,
        "color"=>color,"timestamp"=>Time.now.utc.iso8601,"payload_hash"=>payload_hash
      }
    end
    ...
    def classify(sig)
      f, x, a, y, z = sig["F"], sig["X"], sig["A"], sig["Y"], sig["Z"]
    ...
    def make_labels(sig, pattern)
      labels = []
      %w[P F E A X Y].each do |factor|
        val = sig[factor]
        labels << LABEL_MAP[factor][val] if LABEL_MAP[factor] && LABEL_MAP[factor][val]
      end
      labels << pattern
      labels
    end
