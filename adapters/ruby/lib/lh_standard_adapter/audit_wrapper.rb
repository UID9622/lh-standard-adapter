...
module LongHun
  class AuditWrapper
    ...
    def wrap(payload, task_type: "default", persona: "P04")
      ...
      pattern = classify(signature)
      labels = make_labels(signature, pattern)
      color = determine_color(pattern, signature["R"])

      payload_json = JSON.generate(payload)
      payload_hash = Digest::SHA256.hexdigest(payload_json)[0..15]
    ...
    def classify(sig)
      ...
      return "MODE-Fluctuating" if sig["Z"] > 2.0
